...
public class TooltipChecker extends AbstractSteps {
	...
	public static ArrayList<String> CheckTooltipSpecificatieDeelnemingen(String Naamobject, int Minlengte, int Maxlengte, boolean PosHeelGetal, boolean BSNJa, WebDriver driver) throws Exception {
		Actions action = new Actions(driver);
		TooltipResult.clear();
		Thread.sleep(600);
		// mous-over naar naamobject
		...
		// checker
		checker.clear();
		checker = codebase.ValidatieChecker.CheckValue(invoke(SpecificatieDeelnemingenObjecten.class, Naamobject, driver).getAttribute("value"), Minlengte, Maxlengte, PosHeelGetal, BSNJa);
		if (!checker.isEmpty()) {
			try {
	...
	public static ArrayList<String> CheckTooltipObjectVrijstelling(String Naamobject, int Minlengte, int Maxlengte, boolean PosHeelGetal, boolean BSNJa, WebDriver driver) throws Exception {
		Actions action = new Actions(driver);
		TooltipResult.clear();
		Thread.sleep(600);
		// mous-over naar naamobject
		...
		// checker
		checker.clear();
		checker = codebase.ValidatieChecker.CheckValue(invoke(ObjectvrijstellingObjecten.class, Naamobject, driver).getAttribute("value"), Minlengte, Maxlengte, PosHeelGetal, BSNJa);
		if (!checker.isEmpty()) {
			try {
